...
export default function () {
    ...
    React.useEffect(()=>{
     ...
     const progressOrders =orders.filter((order)=>order.data.status === "In-progress").length;
     const CompletedOrders = orders.filter((order)=>order.data.status === "Completed").length;

     const totalAmount = orders.reduce((sum, order)=> sum + order.data.amount,0);
     const recentOrder = orders.slice(0,2);
    ...
    return (
        <main className="min-h-screen max-w-5xl mx-auto my-10 p-4">
            ...
            <div className="grid grid-cols-1 mb-5 md:grid-cols-2 md:mb-10 lg:grid-cols-4 gap-6 lg:mb-10">
                ...
                <Paper className="p-6 text-center shadow-md rounded-md">
                    ...
                    <p className="text-3xl font-bold text-yellow-300">{progressOrders}</p>
                </Paper>
                <Paper className="p-6 text-center shadow-md rounded-md">
             ...
             <div>
                ...
                <Table>
                    ...
                    <TableBody>
                        ...
                        <TableRow key={orders.id} sx={{borderBottom: "grey"}}>
